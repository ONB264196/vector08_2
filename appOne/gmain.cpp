...
void gmain() {
    ...
    int kotatuColorImg = loadImage("assets\\kotatu2.png");
    int mikanImg = loadImage("assets\\mikan.png");
    int backImg = loadImage("assets\\back.png");

    int kimg = kotatuImg;

    float kpx = width / 2 - 150;
    float kpy = height / 2 + 200;
    ...
    const int  totalNum = 12;
    struct MIKAN m[totalNum];
    int curNum = 0;

    for (int i = 0; i < totalNum; i++) {
        m[i].tx = kpx - 174 + random(-15,15);
        m[i].ty = kpy - 40 - 50*i;
    }
    float speed = 10;
    while (notQuit) {
        ...
        for (int i = 0; i < totalNum; i++) {
            ...
            if (mag > speed) {
                m[i].px = m[i].px + vx / mag * speed;
                m[i].py = m[i].py + vy / mag * speed;
            }
            else {
                m[i].px = m[i].tx;
                m[i].py = m[i].ty;
